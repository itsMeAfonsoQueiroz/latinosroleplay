...
RegisterNetEvent('sd-cokemission:SellItemCheck', function()
    local hasPackages = QBCore.Functions.HasItem(Config.SellItem)
...
local DrawCoord = 1
    ...
    SetCarItemsInfo()
    exports["qb-inventory"]:addTrunkItems(QBCore.Functions.GetPlate(car1), Config.Package)
    exports["qb-inventory"]:addTrunkItems(QBCore.Functions.GetPlate(car2), Config.Package2)
    exports["qb-inventory"]:addTrunkItems(QBCore.Functions.GetPlate(car3), Config.Package3)
...
AddEventHandler('sd-cokemission:client:checkcoke', function()
    local hasCokeBrick = QBCore.Functions.HasItem("coke_brick")

    if hasCokeBrick then
...
AddEventHandler('sd-cokemission:client:checkitem', function()
    local hasSealedPackage = QBCore.Functions.HasItem("sealedpackage")

    if hasSealedPackage then
...
AddEventHandler('sd-blackout:client:bombplant', function()
    local hasC4 = QBCore.Functions.HasItem("c4_bomb")

    if hasC4 then
        TriggerEvent('sd-blackout:client:startblackout')
        Citizen.Wait(1000)
    else
        QBCore.Functions.Notify("You don't have C4!", 'error')
    end    
end)
