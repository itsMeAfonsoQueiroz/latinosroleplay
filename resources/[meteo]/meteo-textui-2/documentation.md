...
CreateThread(function()
    ...
    exports['meteo-textui-2']:create3DTextUI("pa-test", {
    ...
    exports['meteo-textui-2']:delete3DTextUI("pa-test")
    -- For Players
    exports['meteo-textui-2']:create3DTextUIOnPlayers("pa-test", {
        id = targetId,
        ped = targetPed,
        displayDist = 6.0,
        interactDist = 2.0,
    ...
    exports['meteo-textui-2']:delete3DTextUIOnPlayers(targetId)
     -- For Entities, Vehicles, Peds (fixed coordinate)
    exports['meteo-textui-2']:create3DTextUIOnEntity(entity, {
        displayDist = 6.0,
        interactDist = 2.0,
        enableKeyClick = true, -- If true when you near it and click key it will trigger the event that you write inside triggerData
        keyNum = 38,
    ...
    exports['meteo-textui-2']:delete3DTextUI(entity)
end)
```
...
```lua
    ...
    local function drawText(text, position)
        ...
        exports["meteo-textui-2"]:displayTextUI(text, position)
    end

    local function changeText(text, position)
        if type(position) ~= 'string' then position = 'left' end
        exports['meteo-textui-2']:changeText(text, position)
    end
...
```lua
    function ESX.TextUI(message, notifyType)
        if GetResourceState("meteo-textui-2") ~= "missing" then
            return exports["meteo-textui-2"]:displayTextUI(message, notifyType)
        end
        print("[^1ERROR^7] ^5meteo-textui-2^7 is Missing!")
    end

    function ESX.HideUI()
        if GetResourceState("meteo-textui-2") ~= "missing" then
            return exports["meteo-textui-2"]:hideTextUI()
        end
        print("[^1ERROR^7] ^5meteo-textui-2^7 is Missing!")
    end
```