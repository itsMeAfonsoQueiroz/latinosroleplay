    ...
    for k, v in pairs(Config.InvoiceJobs) do
        if k == myJob then
            ...
            if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
                requestActive = true
                exports["pa-textui-2"]:displayTextUI(Lang:t("general.waiting_for_decision"), "ESC")
                for _, id in pairs(nearbyPlayers) do
                    exports['pa-textui-2']:create3DTextUIOnPlayers("pa-billing-request-players-" .. id, {
                        id = id,
                        displayDist = 5.0,
                        interactDist = 1.3,
                        ...
                        theme = "green", -- or red
                        triggerData = {
                            triggerName = "pa-billing:openInvoiceMenu:client",
                            args = {id = id}
                        }
                    })
                end
                Citizen.CreateThread(function()
                    while requestActive do
                        ...
                        if IsControlPressed(0, 322) then
                            ...
                            for _, id in pairs(nearbyPlayers) do
                                exports['pa-textui-2']:delete3DTextUIOnPlayers("pa-billing-request-players-" .. id)
                            end
                        end
                    end
                end)
...
RegisterNetEvent('pa-billing:openInvoiceMenu:client', function(data)
    local myJob = GetPlayerJob()
    local translations = {}
    ...
    TriggerCallback('pa-billing:getTargetName:server', function(name)
...
RegisterNUICallback('callback', function(data)
    ...
    elseif data.action == "sendInvoice" then
        ...
        TriggerServerEvent('pa-billing:sendInvoice:server', data.target, data.price, myJob, data.title)
    elseif data.action == "payBill" then
        TriggerServerEvent('pa-billing:payBill:server', data.id, data.amount, data.type, data.sender)
    end
end)
...
function openBillsMenu()
    TriggerCallback('pa-billing:getMyBills:server', function(myBills)
        --if myBills then
            local translations = {}
            for k in pairs(Lang.fallback and Lang.fallback.phrases or Lang.phrases) do
                if k:sub(0, ('menu.'):len()) then
...
RegisterNetEvent('pa-billing:updateBills:client', function(myBills)
    local data = {}
    for k, v in pairs(myBills) do
        local paid = false
        if tonumber(v.paid) == 1 then
