CreateCallback('pa-billing:getTargetName:server', function(source, cb, id)
    cb(GetCharName(id, true))
end)

CreateCallback('pa-billing:getMyBills:server', function(source, cb)
    local src = source
...
RegisterNetEvent('pa-billing:sendInvoice:server', function(id, price, type, title)
    local src = source
    local me = GetPlayer(src)
    local target = id
...
RegisterNetEvent('pa-billing:payBill:server', function(id, amount, type, sender)
    local src = source
    ...
    if myMoney >= amount then
        ...
        Citizen.Wait(500)
        local myBills = MySQL.query.await('SELECT * FROM pa_billing WHERE receiver = ?', {GetPlayerLicense(src)})
        TriggerClientEvent('pa-billing:updateBills:client', src, myBills)
