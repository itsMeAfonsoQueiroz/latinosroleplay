...
local logData = json.decode(vehicleLogs[1].logs) or {}
local logs = {}
if next(logData) then
...
if CoreName == "qb-core" or CoreName == "qbx_core" then
    Core.Functions.CreateCallback('meteo-garages:server:GetPlayerVehicles', function(source, cb)
        local Player = GetPlayer(source)
        local VehiclesData = {}
...
RegisterNetEvent('meteo-garages:wasabi:impound', function(plate)
    if Table == "player_vehicles" then
