...
local StringCharset = {}
local NumberCharset = {}
RegisterNetEvent('meteo-vehicleshop:buyVehicle:server', function(type, vehicle, price, dealershipId, sender, job)
    local src = source
    ...
    TriggerClientEvent('meteo-vehicleshop:buyVehicle:client', src, vehicle, plate, dealershipId)
...
CreateCallback('meteo-vehicleshop:generatePlate:server', function(source, cb)
    local plate = GeneratePlate()
    cb(plate)
end)
...
-- Stocks
RegisterNetEvent('meteo-vehicleshop:updateDealershipStockData:server', function(dealershipId, stocksData)
    local dealershipData = MySQL.query.await('SELECT * FROM pa_vehicleshop_stocks WHERE dealershipId = ?', {dealershipId})
...
CreateCallback('meteo-vehicleshop:getVehStock:server', function(source, cb, dealershipId)
    local dealershipData = MySQL.query.await('SELECT * FROM pa_vehicleshop_stocks WHERE dealershipId = ?', {dealershipId})
    local anusVal = {}
    for k, v in pairs(dealershipData) do
        for a, b in pairs(v) do
...
local testVehicles = {}
RegisterNetEvent('meteo-vehicleshop:startTest:server')
AddEventHandler('meteo-vehicleshop:startTest:server', function(netId)
    testVehicles[netId] = {
        playerId = source
    }
end)

RegisterNetEvent('meteo-vehicleshop:finishTest:server')
AddEventHandler('meteo-vehicleshop:finishTest:server', function(netId)
...
AddEventHandler('entityRemoved', function(entity)
    ...
    if testVehicles[netId] then
        TriggerClientEvent('meteo-vehicleshop:finishTest:client', testVehicles[netId].playerId)
        testVehicles[netId] = nil
    end
end)

RegisterNetEvent('meteo-vehicleshop:updateShowroomVehicles:server', function(dealershipId, data)
    local src = source
...
CreateCallback('meteo-vehicleshop:getShowroomData:server', function(source, cb, dealershipId)
    local showroomTable = {}
...
RegisterNetEvent('meteo-vehicleshop:sendRequestText:server', function(sender, target, price, model, dealershipId)
    TriggerClientEvent('meteo-vehicleshop:sendRequestText:client', target, sender, price, model, dealershipId)
end)

RegisterNetEvent('meteo-vehicleshop:declinePayment:server', function(sender)
...
AddEventHandler('playerDropped', function()
    for k, v in pairs(testVehicles) do
        if v.playerId == source then
            TriggerClientEvent('meteo-vehicleshop:deleteVehicle:client', -1, k)
        end
    end
end)

--RegisterNetEvent('meteo-vehicleshop:deleteVehicleShowroom:server', function(dealershipId, spotId, newModel, props)
RegisterNetEvent('meteo-vehicleshop:deleteVehicleShowroom:server', function(dealershipId, spotId, newModel)
    for _, playerId in ipairs(GetPlayers()) do
        ...
        if numPlayerId ~= source then
            ...
            if dist <= 40 then
                --TriggerClientEvent('meteo-vehicleshop:deleteVehicleShowroom:client', numPlayerId, dealershipId, spotId, newModel, props)
                TriggerClientEvent('meteo-vehicleshop:deleteVehicleShowroom:client', numPlayerId, dealershipId, spotId, newModel, true)
            else
                TriggerClientEvent('meteo-vehicleshop:deleteVehicleShowroom:client', numPlayerId, dealershipId, spotId, newModel, false)
            end
        end
    end
end)

CreateCallback('meteo-vehicleshop:checkIsPlayerHasPerm:server', function(source, cb)
