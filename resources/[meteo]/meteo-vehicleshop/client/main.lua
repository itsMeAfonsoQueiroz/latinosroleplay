...
Citizen.CreateThread(function()
    ...
    for k, v in pairs(Config.VehicleShops) do
        ...
        if v.Management.Enable then
            exports['meteo-textui-2']:create3DTextUI("meteo-vehicleshop-vehshop-" .. k, {
                ...
                triggerData = {
                    triggerName = "meteo-vehicleshop:openSellingMenu:client",
...
RegisterNetEvent('meteo-vehicleshop:openSellingMenu:client', function(data)
    sellMenuActive = data.open
    if sellMenuActive then
        local showAddStockBtn = false
        TriggerCallback('meteo-vehicleshop:checkIsPlayerHasPerm:server', function(hasPerm)
            if hasPerm then
        ...
        if Config.VehicleShops[data.vehshopId].EnableStocks then
            ...
            TriggerCallback('meteo-vehicleshop:getVehStock:server', function(stockData)
                if stockData ~= 0 then
...
--     Initialized = true
--     Citizen.CreateThread(function()
--         TriggerCallback('meteo-vehicleshop:getShowroomData:server', function(showroomData)
...
--                     exports['meteo-textui-2']:create3DTextUI("meteo-vehicleshop-showroom-vehicle-" .. model .. "-spotId-" .. v.spotId, {
...
--                         theme = "green", 
--                         triggerData = {
--                             triggerName = "meteo-vehicleshop:askAnEmployee:client",
--                             args = {}
--                         }
--                     })
--                 end
...
RegisterNetEvent('meteo-vehicleshop:askAnEmployee:client', function()
...
RegisterNUICallback('callback', function(data)
    ...
    elseif data.action == "close" then
        ...
        elseif data.type == "showroomEdit" then
            ...
            TriggerServerEvent('meteo-vehicleshop:updateShowroomVehicles:server', data.dealershipId, Config.VehicleShops[data.dealershipId].ShowroomVehicles)
        elseif data.type == "sellMenu" then
            TriggerEvent('meteo-vehicleshop:openSellingMenu:client', {open = false})
        else
            SetNuiFocus(false, false)
        end
        if data.test == true and not testDriveState then
            if IsSpawnPointClear(vector3(currentCoords.testVehCoords.x, currentCoords.testVehCoords.y, currentCoords.testVehCoords.z), 2.5) then
                ...
                TriggerServerEvent('meteo-vehicleshop:startTest:server', currentTestVehicleNetId)
    ...
    elseif data.action == "updateShowroomVehicle" then
        ...
        --TriggerServerEvent('meteo-vehicleshop:deleteVehicleShowroom:server', data.dealershipId, data.spotId, data.model, props)
        TriggerServerEvent('meteo-vehicleshop:deleteVehicleShowroom:server', data.dealershipId, data.spotId, data.model)
    ...
    elseif data.action == "createTextsOnPlayers" then
        ...
        if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
            ...
            for _, id in pairs(nearbyPlayers) do
                exports['meteo-textui-2']:create3DTextUIOnPlayers("meteo-vehicleshop-vehshop-players-" .. id, {
                    id = id,
                    displayDist = 5.0,
                    interactDist = 1.3,
                    ...
                    theme = "green", -- or red
                    triggerData = {
                        triggerName = "meteo-vehicleshop:sendSellRequest:client",
            ...
            if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
                for _, id in pairs(nearbyPlayers) do
                    exports['meteo-textui-2']:delete3DTextUIOnPlayers("meteo-vehicleshop-vehshop-players-" .. id)
                end
            end
        else
    ...
    elseif data.action == "confirmPurchase" then
        TriggerServerEvent('meteo-vehicleshop:buyVehicle:server', data.paymentType, data.model, Vehicles2[data.model].price, data.dealershipId, data.sender, Config.VehicleShops[data.dealershipId].Management.Job)
    elseif data.action == "addStockForAllVehicles" then
        ...
        TriggerServerEvent('meteo-vehicleshop:updateDealershipStockData:server', data.dealershipId, stocks)
        taskActive = false
...
RegisterNetEvent('meteo-vehicleshop:sendSellRequest:client', function(data)
    -- Delete All
    if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
        for _, id in pairs(nearbyPlayers) do
            exports['meteo-textui-2']:delete3DTextUIOnPlayers("meteo-vehicleshop-vehshop-players-" .. id)
    ...
    -- Add To Sender
    TriggerServerEvent('meteo-vehicleshop:sendRequestText:server', data.sender, data.target, data.price, data.model, data.dealershipId)
    Citizen.Wait(500)
    taskActive = false
end)

RegisterNetEvent('meteo-vehicleshop:sendRequestText:client', function(sender, price, model, dealershipId)
    exports['meteo-textui-2']:create3DTextUIOnPlayers("meteo-vehicleshop-vehshop-players-confirm-purchase-" .. sender, {
        id = sender,
        displayDist = 5.0,
        interactDist = 1.3,
        enableKeyClick = true,
        ...
        theme = "green",
        triggerData = {
            triggerName = "meteo-vehicleshop:acceptPayment:client",
            args = {price = price, model = model, sender = sender, me = GetPlayerServerId(PlayerId()), dealershipId = dealershipId}
        },
        triggerData2 = {
            triggerName = "meteo-vehicleshop:declinePayment:client",
    ...
    taskActive = false
    Notify("Request timed out.", 7500, "error")
    exports['meteo-textui-2']:delete3DTextUIOnPlayers("meteo-vehicleshop-vehshop-players-confirm-purchase-" .. sender)
end)

RegisterNetEvent('meteo-vehicleshop:acceptPayment:client', function(data)
    Notify("Request accepted.", 7500, "success")
    exports['meteo-textui-2']:delete3DTextUIOnPlayers("meteo-vehicleshop-vehshop-players-confirm-purchase-" .. data.sender)
    TriggerServerEvent('meteo-vehicleshop:buyVehicle:server', "bank", data.model, tonumber(data.price), data.dealershipId, data.sender, Config.VehicleShops[data.dealershipId].Management.Job)
end)

RegisterNetEvent('meteo-vehicleshop:declinePayment:client', function(data)
    Notify("Request declined.", 7500, "error")
    exports['meteo-textui-2']:delete3DTextUIOnPlayers("meteo-vehicleshop-vehshop-players-confirm-purchase-" .. data.sender)
    TriggerServerEvent('meteo-vehicleshop:declinePayment:server', data.sender)
end)
...
AddEventHandler('onResourceStop', function(resource)
    if GetCurrentResourceName() == resource then
        ...
        for k, v in pairs(Config.VehicleShops) do
            ...
            exports['meteo-textui-2']:delete3DTextUI("meteo-vehicleshop-vehshop-" .. k)
        end
    end
end)
...
RegisterNetEvent('meteo-vehicleshop:buyVehicle:client', function(model, plate, dealershipId)
    DoScreenFadeOut(500)
    openDealership(false)
    Citizen.Wait(1000)
...
RegisterNetEvent('meteo-vehicleshop:finishTest:client', function()
    testDriveState = false
...
AddEventHandler('gameEventTriggered', function(event, data)
    if event == "CEventNetworkVehicleUndrivable" then
        ...
        if netId == currentTestVehicleNetId then
            TriggerEvent('meteo-vehicleshop:finishTest:client')
            TriggerServerEvent('meteo-vehicleshop:finishTest:server', netId)
        end
    end
                ...
                for k, v in pairs(Config.VehicleShops) do
                    ...
                    if dist <= 3 then
                        function currentShow()
                            exports["meteo-textui-2"]:displayTextUI("Give Back Test Vehicle", "E")
                            showTextUI = true
                        end
                        function currentHide()
                            showTextUI = false
                            exports["meteo-textui-2"]:hideTextUI()
                        end
				...
				if closestDeliveryPoints.distance < closestDeliveryPoints.maxDist then
					if IsControlJustReleased(0, 38) then
                        ...
                        if VehToNet(veh) == VehToNet(currentTestVehicle) then
                            testDriveState = false
                            SendNUIMessage({action = "stopTestTimer"})
                            TriggerServerEvent('meteo-vehicleshop:finishTest:server', VehToNet(currentTestVehicle))
...
RegisterNetEvent('meteo-vehicleshop:deleteVehicle:client', function(netId)
...
--RegisterNetEvent('meteo-vehicleshop:deleteVehicleShowroom:client', function(dealershipId, spotId, newModel, props)
RegisterNetEvent('meteo-vehicleshop:deleteVehicleShowroom:client', function(dealershipId, spotId, newModel, createNew)
    -- Delete Old
